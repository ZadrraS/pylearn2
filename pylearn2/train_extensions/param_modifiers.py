...
class DropoutScaler(TrainExtension):
    ...
    def setup(self, model, dataset, algorithm):
        ...
        for layer in model.layers[:-1]:
            ...
            channel_name = self.estimate_set + "_" + layer.layer_name
            monitor.add_channel(
                name=channel_name + "_dropout_inc_prob",
                ipt=None,
                val=algorithm.cost.input_include_probs[layer.layer_name],
            ...
            monitor.add_channel(
                ...
                ipt=None,
                val=algorithm.cost.input_scales[layer.layer_name],
                data_specs=(NullSpace(), ''),
                dataset=dataset)
    ...
    def on_monitor(self, model, dataset, algorithm):
        ...
        for i, layer in enumerate(model.layers[:-2]):
            channel_name = self.estimate_set + "_" + layer.layer_name + "_percentage_activated"
            ...
            algorithm.cost.input_include_probs[model.layers[i + 1].layer_name].set_value(np.cast[config.floatX](dropout_include_prob))
            algorithm.cost.input_scales[model.layers[i + 1].layer_name].set_value(np.cast[config.floatX](dropout_scale))
